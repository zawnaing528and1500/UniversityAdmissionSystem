...
while(rs1.next())
{
	...
	l.getConnection();
	if(l.check(currentUser, universityName))
	{
		likeIcon = "like";
	}
	else
	...
	%>
		...
		<div class="box-dark">
			...
			<tr><td><font color="#E9967A" class="bodyText"><%= likeCount %>likes&nbsp<a href="likeAds.jsp?title=<%= universityName %>" class="button cta hvr-float-shadow"><img src="http://localhost:8080/UniversityAdmissionSystem/external/img/icon/<%= likeIcon %>.png"></a></font></td><td><a href="viewComment.jsp?id=<%= id %>&title=<%= universityName%>"><font color="chocolate">view comment</font></a></td></tr>
...
<%
		if(start==0 && end<size){ %>
			<br>
			<a href="member.jsp?start=<%=start+5%>&end=<%=end%>&type=<%= searchType %>&keyword=<%= keyword %>#allAds"><img src="http://localhost:8080/UniversityAdmissionSystem/external/img/icon/next2.png" align="right"></a>
			<%} 
			else if(start!=0 && (start+end)<size){%>
			<a href="member.jsp?start=<%=start-5%>&end=<%=end%>&type=<%= searchType %>&keyword=<%= keyword %>#allAds"><img src="http://localhost:8080/UniversityAdmissionSystem/external/img/icon/previous1.png" align="left"></a>
			<a href="member.jsp?start=<%=start+5%>&end=<%=end%>&type=<%= searchType %>&keyword=<%= keyword %>#allAds"><img src="http://localhost:8080/UniversityAdmissionSystem/external/img/icon/next2.png" align="right"></a>
			<%} 
			else if(start!=0 && (start+end)>size){%>
			<a href="member.jsp?start=<%=start-5%>&end=<%=end%>&type=<%= searchType %>&keyword=<%= keyword %>#allAds"><img src="http://localhost:8080/UniversityAdmissionSystem/external/img/icon/previous1.png" align="left"></a>
